...
public class HashTable {
    ...
    public HashTableElement search(Object key) {
        // you must provide
        LinkedList<HashTableElement> containChain = chains.get(hash(key));
        
        for(int i = 0; i < containChain.size(); i++){
            if(containChain.get(i).getKey().equals(key))
                return containChain.get(i);
        }
        return null;
    }
    ...
    public boolean remove(HashTableElement element) {
        // you must provide
        LinkedList<HashTableElement> containChain = chains.get(hash(element.getKey()));
        boolean boolVal = containChain.remove(element);
        
        if(boolVal){
            int newInt = chainLengths.get(hash(element.getKey()));
            chainLengths.set(hash(element.getKey()), newInt-1);
            size --;
        }
        return boolVal;
    }
    ...
    private static int stringHash(String key) {
        ...
        int i = 0;
        while(i < key.length()){
            intHash += key.codePointAt(i);
            i++;
        }
        return integerHash(intHash);
    }
